...
import { Injector, Logger } from '@majel/core'
 ...
 *
 * ```ts
 * import { EmailEventListener } from `\@majel/email-plugin`;
...
export class EmailEventHandler<T extends string = string, Event extends EventWithContext = EventWithContext> {
	...
	private getBestConfiguration(
		...
		const exactMatch = this.configurations.find(c => {
			return (
				(c.channelCode === channelCode || c.channelCode === 'default') && c.languageCode === languageCode
			)
		})
		if (exactMatch) {
			return exactMatch
