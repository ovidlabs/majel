...
import { CurrencyCode, SortOrder } from '@majel/common/lib/generated-types'
...
import { createTestEnvironment, E2E_DEFAULT_CHANNEL_TOKEN } from '@majel/testing'
...
describe('Elasticsearch plugin', () => {
				...
				ElasticsearchPlugin.init({
					...
					customProductVariantMappings: {
						...
						materialCode: {
							graphQlType: 'String!',
							valueFn: variant => {
								const materialFacetValue: FacetValue | undefined = (variant.customFields as any)
									.material
								let result = ''
	...
	describe('admin api', () => {
		...
		describe('updating the index', () => {
			...
			it('updates index when a Product is deleted', async () => {
				...
				expect(search.items.map(i => i.productId).sort()).toEqual(['T_2', 'T_3', 'T_4', 'T_5', 'T_6'])
			...
			describe('asset changes', () => {
				...
				it('updates index when asset focalPoint is changed', async () => {
					...
					await adminClient.query<Codegen.UpdateAssetMutation, Codegen.UpdateAssetMutationVariables>(
						UPDATE_ASSET,
						{
							input: {
								id: 'T_1',
				...
				it('updates index when asset deleted', async () => {
					...
					await adminClient.query<Codegen.DeleteAssetMutation, Codegen.DeleteAssetMutationVariables>(
						DELETE_ASSET,
						{
							input: {
								assetId: assetId!,
		...
		describe('channel handling', () => {
			...
			it('reindexes in channel', async () => {
				...
				const { job } = await adminClient.query<GetJobInfoQuery, GetJobInfoQueryVariables>(GET_JOB_INFO, {
					id: reindex.id,
				})
	...
	describe('customMappings', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/1638
