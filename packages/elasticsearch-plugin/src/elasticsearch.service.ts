...
import { SearchResultAsset } from '@majel/common/lib/generated-types'
import {
	Collection,
	CollectionService,
	ConfigService,
...
export class ElasticsearchService implements OnModuleInit, OnModuleDestroy {
		...
		await new Promise<void>(async (resolve, reject) => {
			...
			while (attempts < connectionAttempts) {
				...
				try {
					...
					Logger.verbose(`Ping to Elasticsearch failed with error "${e.message as string}"`, loggerCtx)
				}
				Logger.verbose(
		...
		const createIndex = async (indexName: string) => {
			...
			if (!result.body) {
				...
				const existingIndexSettings =
					existingIndexSettingsResult.body[Object.keys(existingIndexSettingsResult.body)[0]].settings
						.index
				...
				if (!equal(tempIndexSettings, existingIndexSettings))
					Logger.warn(
						`Index "${index}" settings differs from index setting in majel config! Consider re-indexing the data.`,
						loggerCtx,
					)
				else {
					const existingIndexMappingsResult = await this.client.indices.getMapping({ index })
					const existingIndexMappings =
						existingIndexMappingsResult.body[Object.keys(existingIndexMappingsResult.body)[0]].mappings
					...
					if (!equal(tempIndexMappings, existingIndexMappings))
						...
						Logger.warn(
							`Index "${index}" mapping differs from index mapping in majel config! Consider re-indexing the data.`,
							loggerCtx,
						)
				}
		...
		if (groupByProduct) {
			try {
				...
				if (e.meta.body.error.type && e.meta.body.error.type === 'search_phase_execution_exception') {
					...
					Logger.error(e.message, loggerCtx, JSON.stringify(e.meta.body.error.root_cause || [], null, 2))
	...
	async priceRange(ctx: RequestContext, input: ElasticSearchInput): Promise<SearchPriceData> {
		const { indexPrefix, searchConfig } = this.options
		const elasticSearchBody = buildElasticBody(input, searchConfig, ctx.channelId, ctx.languageCode, true)
		elasticSearchBody.from = 0
		elasticSearchBody.size = 0
		elasticSearchBody.aggs = {
			minPrice: {
