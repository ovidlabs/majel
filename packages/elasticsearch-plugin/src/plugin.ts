...
function getCustomResolvers(options: ElasticsearchRuntimeOptions) {
	...
	const requiresUnionScriptResolver =
		0 <
			Object.values(options.searchConfig.scriptFields || {}).filter(field => field.context !== 'product')
				.length &&
		0 <
			Object.values(options.searchConfig.scriptFields || {}).filter(field => field.context !== 'variant')
				.length
	return [
 ...
 *
 * `yarn add \@elastic/elasticsearch \@majel/elasticsearch-plugin`
 *
 * or
 *
 * `npm install \@elastic/elasticsearch \@majel/elasticsearch-plugin`
 *
 * Make sure to remove the `DefaultSearchPlugin` if it is still in the MajelConfig plugins array.
 *
