...
async function pushAppConfig(orgId, schema) {
	...
	const result = await fetch(apiUrl, {
		...
		body: JSON.stringify({
			query: /* GraphQL */ `
				mutation ($input: Input!, $appName: String!, $schema: IntrospectionQuery) {
					pushAppConfig(input: $input, appName: $appName, schema: $schema, allowDeletion: true)
				}
			`,
			variables: {
				input: config,
...
async function createApp(orgId, schema) {
	...
	const result = await fetch(apiUrl, {
		...
		body: JSON.stringify({
			query: /* GraphQL */ `
				mutation ($input: Input!, $schema: IntrospectionQuery, $organizationId: String!) {
					createAppCli(input: $input, schema: $schema, organizationId: $organizationId) {
						id
						config {
							input
						}
