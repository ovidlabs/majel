...
import { Collection, ID, Logger, Product, ProductVariant } from '@majel/core'
...
export class StellateService {
	...
	async purgeProductVariants(productVariants: ProductVariant[]) {
		Logger.verbose(`Purging cache: ProductVariant(${productVariants.map(p => p.id).join(', ')})`, loggerCtx)
		await this.purge(
			'ProductVariant',
