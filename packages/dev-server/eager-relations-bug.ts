...
import { MajelEntity, EntityId, ID, OrderLine } from '@majel/core'
import gql from 'graphql-tag'
import { Column, Entity, ManyToOne } from 'typeorm'

@Entity()
export class CutCode extends MajelEntity {
...
export class EagerRelationsBugOrderResolver {
	...
	async addCutToOrder(@Ctx() ctx: RequestContext): Promise<Order | null> {
		...
		let cutCode = await this.connection.getRepository(ctx, CutCode).findOne({ where: { code: 'cut-code' } })
		...
		await this.customFieldRelationService.updateRelations(ctx, OrderLine, { customFields }, savedOrderLine)
