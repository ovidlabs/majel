...
export class SimpleGraphQLClient {
	...
	constructor(
		private majelConfig: Required<MajelConfig>,
	...
	setChannelToken(token: string | null) {
		...
		if (this.majelConfig.apiOptions.channelTokenKey) {
			this.headers[this.majelConfig.apiOptions.channelTokenKey] = this.channelToken
		}
	}
	...
	async fetch(url: string, options: RequestInit = {}): Promise<Response> {
		...
		const authToken = response.headers.get(this.majelConfig.authOptions.authTokenHeaderKey || '')
		if (authToken != null) {
	 ...
	 */
	async asSuperAdmin() {
		const { superadminCredentials } = this.majelConfig.authOptions
