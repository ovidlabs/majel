 ...
 * @example
 * ```ts title="routes.ts"
 * import { registerRouteComponent } from '\@majel/admin-ui/core';
 * import { registerReactRouteComponent } from '\@majel/admin-ui/react';
 *
	...
	return {
		...
		data: {
			...
			...(getBreadcrumbs && query && entityKey
				? {
						breadcrumb: data => data.detail.entity.pipe(map((entity: any) => getBreadcrumbs(entity))),
					}
				: {}),
			...(options.routeConfig?.data ?? {}),
		},
