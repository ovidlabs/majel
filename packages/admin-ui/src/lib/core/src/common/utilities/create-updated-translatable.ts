import { CustomFieldsObject, CustomFieldType } from '@majel/common/lib/shared-types'
...
export function createUpdatedTranslatable<T extends { translations: any[] } & MayHaveCustomFields>(
	...
	const currentTranslation = findTranslation(translatable, languageCode) || defaultTranslation || ({} as any)
		...
		for (const field of customFieldConfig) {
			...
			if (field.type === 'localeString' || field.type === 'localeText') {
				...
				newCustomFields[field.name] =
					value === '' ? getDefaultValue(field.type as CustomFieldType, field.nullable ?? true) : value
			}
		}
