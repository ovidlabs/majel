...
import { DataService, GetOrderStateQuery, GetOrderStateQueryVariables } from '@majel/admin-ui/core'
		...
		if (isDraft) {
			if (id === 'create') {
				return this.dataService.order
					.createDraftOrder()
					.pipe(
						map(({ createDraftOrder }) => this.router.parseUrl(`/orders/draft/${createDraftOrder.id}`)),
					)
			} else {
				return true
			}
