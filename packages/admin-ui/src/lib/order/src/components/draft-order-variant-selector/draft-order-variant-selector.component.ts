...
import { CurrencyCode, CustomFieldConfig, DataService, GetProductVariantQuery } from '@majel/admin-ui/core'
	...
	addItemClick(selectedVariant: GetProductVariantQuery['productVariant']) {
		if (selectedVariant) {
			this.addItem.emit({
				...
				customFields: this.orderLineCustomFields.length ? this.customFieldsFormGroup.value : undefined,
			})
