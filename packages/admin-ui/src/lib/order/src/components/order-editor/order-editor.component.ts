...
import { assertNever, notNullOrUndefined } from '@majel/common/lib/shared-utils'
	...
	implements OnInit, OnDestroy
{
	...
	addItemToOrder(result: ProductSelectorItem | undefined) {
		...
		const customFields = this.orderLineCustomFields.length ? this.addItemCustomFieldsForm.value : undefined
	...
	previewAndModify(order: OrderDetailFragment) {
		...
		const input: ModifyOrderInput = {
			...
			...(this.shippingAddressForm.dirty ? { updateShippingAddress: this.shippingAddressForm.value } : {}),
			dryRun: true,
		...
		if (Object.entries(this.updatedShippingMethods).length) {
			input.shippingMethodIds = order.shippingLines.map(l =>
				this.updatedShippingMethods[l.id] ? this.updatedShippingMethods[l.id].id : l.shippingMethod.id,
			)
		}
		this.dataService.order
			.modifyOrder(input)
