...
export class LoginComponent implements OnInit {
	...
	loadImage() {
		this.httpClient
			.get('https://login-image.majel.io')
			.toPromise()
			.then(res => {
	...
	updateImage(res: any) {
		...
		this.imageUrl = res.urls.regular + '?utm_source=Majel+Login+Image&utm_medium=referral'
		this.imageCreator = user.name
		this.imageLocation = location.name
		this.imageCreatorUrl = user.links.html + '?utm_source=Majel+Login+Image&utm_medium=referral'
