	...
	implements OnInit, OnDestroy
{
			...
			.pipe(
				...
				mergeMap(({ id }) => {
					...
					if (customerForm && customerForm.dirty) {
						...
						saveOperations.push(
							this.dataService.customer.updateCustomer(customer).pipe(map(res => res.updateCustomer)),
						)
					}
					const addressFormArray = this.detailForm.get('addresses') as UntypedFormArray
	...
	removeFromGroup(group: CustomerWithOrders['groups'][number]) {
		this.modalService
			...
			.pipe(
				switchMap(response =>
					response ? this.dataService.customer.removeCustomersFromGroup(group.id, [this.id]) : EMPTY,
				),
