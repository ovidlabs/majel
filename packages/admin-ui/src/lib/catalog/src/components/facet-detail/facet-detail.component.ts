...
import { SortOrder } from '@majel/common/lib/generated-types'
	...
	implements OnInit, OnDestroy
{
	...
	save() {
		const valuesFormRecord = this.detailForm.get('values') as (typeof this.detailForm)['controls']['values']
	...
	private setFacetValueFormValues(facet: FacetWithValueListFragment, languageCode: LanguageCode) {
		...
		facet.valueList.items.forEach(value => {
			...
			if (this.customValueFields.length) {
				let customValueFieldsGroup = valueControl.get(['customFields']) as UntypedFormGroup | undefined
