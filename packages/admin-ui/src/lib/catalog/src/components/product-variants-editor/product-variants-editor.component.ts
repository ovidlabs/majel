...
export class ProductVariantsEditorComponent implements OnInit, DeactivateAware {
	...
	ngOnInit() {
		this.languageCode = (this.route.snapshot.paramMap.get('lang') as LanguageCode) || getDefaultUiLanguage()
	...
	deleteVariant(variant: NonNullable<GetProductVariantOptionsQuery['product']>['variants'][number]) {
		this.modalService
			...
			.pipe(
				switchMap(response =>
					response ? this.productDetailService.deleteProductVariant(variant.id, this.product.id) : EMPTY,
				),
			)
			.subscribe(
				() => {
