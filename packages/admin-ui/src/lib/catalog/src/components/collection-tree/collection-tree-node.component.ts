...
import { DataService, Permission, SelectionManager } from '@majel/admin-ui/core'
...
export class CollectionTreeNodeComponent implements OnInit, OnChanges, OnDestroy {
	...
	ngOnInit() {
		...
		this.hasUpdatePermission$ = permissions$.pipe(
			map(
				perms => perms.includes(Permission.UpdateCatalog) || perms.includes(Permission.UpdateCollection),
			),
		)
		this.hasDeletePermission$ = permissions$.pipe(
			map(
				perms => perms.includes(Permission.DeleteCatalog) || perms.includes(Permission.DeleteCollection),
			),
		)
