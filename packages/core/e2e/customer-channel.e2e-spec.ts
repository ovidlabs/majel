...
describe('ChannelAware Customers', () => {
	...
	describe('Address manipulation', () => {
		it(
			...
			assertThrowsWithMessage(async () => {
				...
				await adminClient.query<Codegen.UpdateAddressMutation, Codegen.UpdateAddressMutationVariables>(
					UPDATE_ADDRESS,
					{
						input: {
							id: 'T_1',
		...
		it(
			...
			assertThrowsWithMessage(async () => {
				adminClient.setChannelToken(SECOND_CHANNEL_TOKEN)
				await adminClient.query<Codegen.CreateAddressMutation, Codegen.CreateAddressMutationVariables>(
					CREATE_ADDRESS,
					{
						id: firstCustomer.id,
						input: {
	...
	describe('Customer manipulation', () => {
		...
		it(
			...
			assertThrowsWithMessage(async () => {
				adminClient.setChannelToken(SECOND_CHANNEL_TOKEN)
				await adminClient.query<Codegen.UpdateCustomerMutation, Codegen.UpdateCustomerMutationVariables>(
					UPDATE_CUSTOMER,
					{
						input: {
							id: firstCustomer.id,
		...
		it('brings customer to current channel when creating with existing emailAddress', async () => {
			...
			expect(customersSecondChannel.customers.items.map(customer => customer.emailAddress)).not.toContain(
				firstCustomer.emailAddress,
			)
	...
	describe('Shop API', () => {
		it('assigns authenticated customers to the channels they visit', async () => {
			...
			expect(customers.items.map(customer => customer.emailAddress)).toContain(secondCustomer.emailAddress)
		})
		...
		// https://github.com/majel-ecommerce/majel/issues/834
		it('handles concurrent assignments to a new channel', async () => {
			...
			try {
				await Promise.all([shopClient.query<Codegen.MeQuery>(ME), shopClient.query<Codegen.MeQuery>(ME)])
			} catch (e: any) {
