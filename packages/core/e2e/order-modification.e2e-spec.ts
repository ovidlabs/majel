...
describe('Order modification', () => {
	...
	const orderGuard: ErrorResultGuard<UpdatedOrderFragment | OrderWithModificationsFragment | OrderFragment> =
	...
	beforeAll(async () => {
		...
		>(CREATE_SHIPPING_METHOD, {
			input: {
				...
				translations: [{ languageCode: LanguageCode.en, name: 'test method express', description: '' }],
			},
		})
		...
		it('addItems with existing variant id increments existing OrderLine', async () => {
			...
			>(MODIFY_ORDER, {
				input: {
					dryRun: true,
					orderId,
					addItems: [{ productVariantId: 'T_1', quantity: 1, customFields: { color: 'green' } } as any],
				},
			})
			orderGuard.assertSuccess(modifyOrder)
		...
		it('addItems with existing variant id but different customFields adds new OrderLine', async () => {
			...
			>(MODIFY_ORDER, {
				input: {
					dryRun: true,
					orderId,
					addItems: [{ productVariantId: 'T_1', quantity: 1, customFields: { color: 'blue' } } as any],
				},
			})
			orderGuard.assertSuccess(modifyOrder)
	...
	describe('wet run', () => {
		...
		it('update Order customFields', async () => {
			...
			const { order: orderWithCustomFields } = await adminClient.query(gql(GET_ORDER_WITH_CUSTOM_FIELDS), {
				id: order.id,
			})
	...
	// https://github.com/majel-ecommerce/majel/issues/1753
	...
	// https://github.com/majel-ecommerce/majel/issues/688 - 4th point
	...
	// https://github.com/majel-ecommerce/majel/issues/872
	...
	describe('refund handling when promotions are active on order', () => {
		// https://github.com/majel-ecommerce/majel/issues/890
		it('refunds correct amount when order-level promotion applied', async () => {
			...
			expect(modifyOrder.totalWithTax).toBe(originalTotalWithTax - order.lines[0].proratedUnitPriceWithTax)
			expect(modifyOrder.payments![0].refunds[0].total).toBe(order.lines[0].proratedUnitPriceWithTax)
			expect(modifyOrder.totalWithTax).toBe(getOrderPaymentsTotalWithRefunds(modifyOrder))
		})

		// https://github.com/majel-ecommerce/majel/issues/1865
		describe('issue 1865', () => {
	...
	// https://github.com/majel-ecommerce/majel/issues/1197
	...
	// https://github.com/majel-ecommerce/majel/issues/1210
