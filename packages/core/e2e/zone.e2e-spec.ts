...
describe('Zone resolver', () => {
	...
	it('zone', async () => {
		const result = await adminClient.query<Codegen.GetZoneQuery, Codegen.GetZoneQueryVariables>(GET_ZONE, {
			id: oceania.id,
		})
	...
	it('updateZone', async () => {
		const result = await adminClient.query<Codegen.UpdateZoneMutation, Codegen.UpdateZoneMutationVariables>(
			UPDATE_ZONE,
			{
				input: {
					id: oceania.id,
	...
	it('createZone', async () => {
		const result = await adminClient.query<Codegen.CreateZoneMutation, Codegen.CreateZoneMutationVariables>(
			CREATE_ZONE,
			{
				input: {
					name: 'Pangaea',
