...
} from '@majel/common/lib/shared-constants'
...
describe('Role resolver', () => {
	...
	it('roles', async () => {
		const result = await adminClient.query<Codegen.GetRolesQuery, Codegen.GetRolesQueryVariables>(GET_ROLES)
	...
	it('createRole with permissions', async () => {
		const result = await adminClient.query<Codegen.CreateRoleMutation, Codegen.CreateRoleMutationVariables>(
			CREATE_ROLE,
			{
				input: {
					code: 'test',
	...
	it('role', async () => {
		const result = await adminClient.query<Codegen.GetRoleQuery, Codegen.GetRoleQueryVariables>(GET_ROLE, {
			id: createdRole.id,
		})
	...
	// https://github.com/majel-ecommerce/majel/issues/1874
