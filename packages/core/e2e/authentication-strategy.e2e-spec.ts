...
import { ErrorCode } from '@majel/common/lib/generated-shop-types'
import { pick } from '@majel/common/lib/pick'
import { mergeConfig, NativeAuthenticationStrategy } from '@majel/core'
...
describe('AuthenticationStrategy', () => {
	...
	describe('external auth', () => {
		...
		it('creates a new Customer with valid token', async () => {
			...
			expect(after.items.map(i => i.emailAddress)).toEqual(['hayden.zieme12@hotmail.com', userData.email])
		...
		// https://github.com/majel-ecommerce/majel/issues/695
		it('multiple external auth strategies to not interfere with one another', async () => {
			...
			const { customers: customers1 } = await adminClient.query<Codegen.GetCustomersQuery>(GET_CUSTOMERS)
			expect(customers1.items).toEqual(EXPECTED_CUSTOMERS)
			const { authenticate: auth1 } = await shopClient.query<Codegen.AuthenticateMutation>(AUTHENTICATE, {
				input: {
					test_strategy2: {
						token: VALID_AUTH_TOKEN,
			...
			const { customers: customers2 } = await adminClient.query<Codegen.GetCustomersQuery>(GET_CUSTOMERS)
			expect(customers2.items).toEqual(EXPECTED_CUSTOMERS)

			await shopClient.asAnonymousUser()

			const { authenticate: auth2 } = await shopClient.query<Codegen.AuthenticateMutation>(AUTHENTICATE, {
				input: {
					test_strategy: {
						token: VALID_AUTH_TOKEN,
						userData,
			...
			const { customers: customers3 } = await adminClient.query<Codegen.GetCustomersQuery>(GET_CUSTOMERS)
		...
		// https://github.com/majel-ecommerce/majel/issues/926
	...
	describe('native auth', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/486#issuecomment-704991768
...
describe('No NativeAuthStrategy on Shop API', () => {
	...
	// https://github.com/majel-ecommerce/majel/issues/2282
