...
describe('Entity hydration', () => {
	...
	// https://github.com/majel-ecommerce/majel/issues/1153
	...
	// https://github.com/majel-ecommerce/majel/issues/1324
	...
	// https://github.com/majel-ecommerce/majel/issues/1161
	...
	// https://github.com/majel-ecommerce/majel/issues/1172
	...
	// https://github.com/majel-ecommerce/majel/issues/1229
	...
	// https://github.com/majel-ecommerce/majel/issues/1284
	...
	// https://github.com/majel-ecommerce/majel/issues/2682
	...
	// https://github.com/majel-ecommerce/majel/issues/2013
	describe('hydration of OrderLine ProductVariantPrices', () => {
		...
		it('Create order with 3 items', async () => {
			...
			// This is ugly, but in our real life example we use a CTX constructed by Majel
	...
	// https://github.com/majel-ecommerce/majel/issues/2546
	it('Preserves ordering when merging arrays of relations', async () => {
		...
		const order = await server.app.get(OrderService).findOne(ctx, internalOrderId, ['lines.productVariant'])
		...
		await server.app.get(TransactionalConnection).getRepository(ctx, OrderLine).update(order!.lines[0].id, {
			sellerChannelId: 1,
		})
