...
import { notNullOrUndefined } from '@majel/common/lib/shared-utils'
import { createErrorResultGuard, createTestEnvironment, ErrorResultGuard } from '@majel/testing'
...
describe('Product resolver', () => {
	...
	describe('product query', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/820
		...
		// https://github.com/majel-ecommerce/majel/issues/538
		...
		it(
			...
			assertThrowsWithMessage(async () => {
				await adminClient.query<Codegen.GetProductSimpleQuery, Codegen.GetProductSimpleQueryVariables>(
					GET_PRODUCT_SIMPLE,
					{},
				)
		...
		describe('product query with translations', () => {
			...
			beforeAll(async () => {
				...
				en_translation = translatedProduct.translations.find(t => t.languageCode === LanguageCode.en)!
				de_translation = translatedProduct.translations.find(t => t.languageCode === LanguageCode.de)!
			})
	...
	describe('product mutation', () => {
		...
		it('updateProduct accepts partial input', async () => {
			...
			expect(result.updateProduct.translations.find(t => t.languageCode === LanguageCode.de)!.name).toBe(
				'de Mashed Potato',
			)
			expect(result.updateProduct.translations.find(t => t.languageCode === LanguageCode.en)!.name).toBe(
				'en Very Mashed Potato',
			)
			expect(
		...
		describe('variants', () => {
			...
			// https://github.com/majel-ecommerce/majel/issues/1101
			...
			describe('adding options to existing variants', () => {
				...
				it('addOptionGroupToProduct and then update existing ProductVariant with a new option', async () => {
					const optionGroup4 = await createOptionGroup('group-4', ['group4-option-1', 'group4-option-2'])
					...
					>(UPDATE_PRODUCT_VARIANTS, {
						input: [
							{
								id: variantToModify.id,
								optionIds: [...variantToModify.options.map(o => o.id), optionGroup4.options[0].id],
							},
						],
					})
				...
				it('removeOptionGroup with force argument', async () => {
					...
					function assertNoOptionGroup(variant: Codegen.ProductVariantFragment, optionGroupId: string) {
						expect(variant.options.map(o => o.groupId).every(id => id !== optionGroupId)).toBe(true)
					}
			...
			/** Testing https://github.com/majel-ecommerce/majel/issues/412 **/
			...
			// https://github.com/majel-ecommerce/majel/issues/980
			...
			// https://github.com/majel-ecommerce/majel/issues/1631
	...
	describe('deletion', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/1096
		...
		// https://github.com/majel-ecommerce/majel/issues/558
		...
		// https://github.com/majel-ecommerce/majel/issues/1505
		...
		// https://github.com/majel-ecommerce/majel/issues/800
