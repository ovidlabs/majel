...
describe('Promotions applied to Orders', () => {
	...
	describe('coupon codes', () => {
		...
		it('coupon code application is case-sensitive', async () => {
			...
			orderResultGuard.assertErrorResult(applyCouponCode)
			expect(applyCouponCode.message).toBe(`Coupon code "${TEST_COUPON_CODE.toLowerCase()}" is not valid`)
			expect(applyCouponCode.errorCode).toBe(ErrorCode.COUPON_CODE_INVALID_ERROR)
		})
		...
		// https://github.com/majel-ecommerce/majel/issues/649
		...
		describe('coupon codes in other channels', () => {
			...
			// https://github.com/majel-ecommerce/majel/issues/1692
	...
	describe('default PromotionConditions', () => {
		...
		it('atLeastNWithFacets', async () => {
			...
			expect(res2.totalWithTax).toBe(0)
			expect(res2.discounts[0].description).toBe('Free if order contains 2 items with Sale facet value')
	...
	describe('default PromotionActions', () => {
		...
		describe('orderPercentageDiscount', () => {
			...
			// https://github.com/majel-ecommerce/majel/issues/1773
		...
		describe('freeShipping', () => {
			...
			// https://github.com/majel-ecommerce/majel/pull/1150
		...
		describe('multiple promotions simultaneously', () => {
			...
			it('prices exclude tax', async () => {
				...
				expect(saleItemLine.discounts.find(a => a.type === AdjustmentType.PROMOTION)?.description).toBe(
					'item promo',
				)
	...
	describe('per-customer usage limit', () => {
		...
		describe('guest customer', () => {
			...
			it('adds Promotions to Order once payment arranged', async () => {
				...
				expect(orderByCode!.promotions.map(pick(['name']))).toEqual([{ name: 'Free with test coupon' }])
			})
			...
			it('returns error result when usage exceeds limit', async () => {
				...
				expect(applyCouponCode.message).toEqual('Coupon code cannot be used more than once per customer')
			})
		...
		describe('signed-in customer', () => {
			...
			// https://github.com/majel-ecommerce/majel/issues/1466
	...
	// https://github.com/majel-ecommerce/majel/issues/710
	...
	// https://github.com/majel-ecommerce/majel/issues/1492
	...
	// https://github.com/majel-ecommerce/majel/issues/2385
