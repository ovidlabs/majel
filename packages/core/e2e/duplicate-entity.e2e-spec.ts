...
describe('Duplicating entities', () => {
	...
	describe('default entity duplicators', () => {
		describe('Product duplicator', () => {
			...
			let originalFirstVariant: NonNullable<Codegen.GetProductWithVariantsQuery['product']>['variants'][0]
			...
			it('variant assets are preserved', async () => {
				...
				expect(product?.variants.find(v => v.name === originalFirstVariant.name)?.featuredAsset).toEqual(
					originalFirstVariant.featuredAsset,
				)
			})
			...
			it('variant facet values are preserved', async () => {
				...
				expect(product?.variants.find(v => v.name === originalFirstVariant.name)?.facetValues).toEqual(
					originalFirstVariant.facetValues,
				)
			})
			...
			it('variant stock levels are preserved', async () => {
				...
				expect(product?.variants.find(v => v.name === originalFirstVariant.name)?.stockOnHand).toBe(100)
			})
			...
			it('tax categories are duplicated', async () => {
				...
				const variant = productReloaded?.variants.find(v => v.sku.startsWith(product!.variants[0].sku))
				expect(variant).not.toBeUndefined()
		...
		describe('Facet duplicator', () => {
			...
			it('facet values are duplicated', async () => {
				...
				expect(facet?.values.map(v => v.name).sort()).toEqual(['computers (copy)', 'electronics (copy)'])
			})
		})
