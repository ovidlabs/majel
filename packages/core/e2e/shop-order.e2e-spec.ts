...
describe('Shop orders', () => {
	...
	it('availableCountries returns enabled countries', async () => {
		...
		const result = await shopClient.query<CodegenShop.GetAvailableCountriesQuery>(GET_AVAILABLE_COUNTRIES)
	...
	describe('ordering as anonymous user', () => {
		...
		describe('OrderLine customFields', () => {
			...
			// https://github.com/majel-ecommerce/majel/issues/1670
		...
		// https://github.com/majel-ecommerce/majel/issues/2702
	...
	describe('ordering as authenticated user', () => {
		...
		describe('shipping', () => {
			...
			it('setOrderShippingMethod sets the shipping method', async () => {
				...
				expect(order.shippingLines[0].shippingMethod.description).toBe(shippingMethods[1].description)
			})
		...
		describe('payment', () => {
			...
			it('adds an error payment and returns error result', async () => {
				...
				const result =
					await shopClient.query<CodegenShop.GetActiveOrderPaymentsQuery>(GET_ACTIVE_ORDER_PAYMENTS)
	...
	describe('order merging', () => {
		...
		/**
		 * See https://github.com/majel-ecommerce/majel/issues/263
		 */
		...
		// https://github.com/majel-ecommerce/majel/issues/754
	...
	describe('validation of product variant availability', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/1567
	...
	// https://github.com/majel-ecommerce/majel/issues/1195
	describe('shipping method invalidation', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/1441
		it('shipping methods are re-evaluated when all OrderLines are removed', async () => {
			...
			>(CREATE_SHIPPING_METHOD, {
				input: {
					...
					translations: [{ languageCode: LanguageCode.en, name: 'min price shipping', description: '' }],
					fulfillmentHandler: manualFulfillmentHandler.code,
					checker: {
	...
	describe('edge cases', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/2548
		it('hydrating Order in the ShippingEligibilityChecker does not break order modification', async () => {
			...
			await adminClient.query(CreateShippingMethodDocument, {
				input: {
					...
					translations: [{ languageCode: LanguageCode.en, name: 'hydrating checker', description: '' }],
					fulfillmentHandler: manualFulfillmentHandler.code,
					checker: {
