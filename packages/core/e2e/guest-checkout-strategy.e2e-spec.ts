...
class TestGuestCheckoutStrategy implements GuestCheckoutStrategy {
	...
	async setCustomerForOrder(
		...
		if (TestGuestCheckoutStrategy.createNewCustomerOnEmailAddressConflict === true) {
			...
			if (existing) {
				const newCustomer = await this.connection.getRepository(ctx, Customer).save(new Customer(input))
...
describe('Order taxes', () => {
	...
	const orderResultGuard: ErrorResultGuard<CodegenShop.ActiveOrderCustomerFragment> = createErrorResultGuard(
		input => !!input.lines,
	)
