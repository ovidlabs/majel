...
describe('Default search plugin', () => {
	...
	async function testSortingNoGrouping(testProducts: TestProducts, sortBy: keyof SearchResultSortParameter) {
		const result = await testProducts({
			groupByProduct: false,
			sort: {
	...
	describe('shop api', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/1236
		...
		it('omits facetValues of private facets', async () => {
			const { createFacet } = await adminClient.query<CreateFacetMutation, CreateFacetMutationVariables>(
				CREATE_FACET,
				{
					input: {
						code: 'profit-margin',
		...
		it('returns correct collections when not grouped by product', async () => {
			...
			expect(result.search.collections).toEqual([{ collection: { id: 'T_2', name: 'Plants' }, count: 3 }])
		})
		...
		it('encodes the productId and productVariantId', async () => {
			const result = await shopClient.query<SearchProductsShopQuery, SearchProductsShopQueryVariablesExt>(
				SEARCH_PRODUCTS_SHOP,
				{
					input: {
						groupByProduct: false,
	...
	describe('admin api', () => {
		...
		describe('updating the index', () => {
			...
			it('updates index when a Product is deleted', async () => {
				...
				await adminClient.query<DeleteProductMutation, DeleteProductMutationVariables>(DELETE_PRODUCT, {
					id: 'T_5',
				})
				await awaitRunningJobs(adminClient)
				const { search: search2 } = await testProductsAdmin({
			...
			it('updates index when a taxRate is changed', async () => {
				await adminClient.query<UpdateTaxRateMutation, UpdateTaxRateMutationVariables>(UPDATE_TAX_RATE, {
					input: {
			...
			it('does not include deleted ProductVariants in index', async () => {
				...
				const { search } = await adminClient.query<SearchGetPricesQuery, SearchGetPricesQueryVariables>(
					SEARCH_GET_PRICES,
			...
			// https://github.com/majel-ecommerce/majel/issues/295
			...
			// https://github.com/majel-ecommerce/majel/issues/1482
			...
			// https://github.com/majel-ecommerce/majel/issues/745
			it('very long Product descriptions no not cause indexing to fail', async () => {
				...
				expect(result.search.items.map(i => i.productName)).toEqual(['Very long description aabbccdd'])
				await adminClient.query<DeleteProductMutation, DeleteProductMutationVariables>(DELETE_PRODUCT, {
					id: createProduct.id,
				})
			})
		...
		// https://github.com/majel-ecommerce/majel/issues/609
			...
			it('creates synthetic index item for Product with no variants', async () => {
				...
				expect(
					result.search.items.map(
						pick(['productId', 'enabled', 'productName', 'productVariantName', 'slug', 'description']),
					),
				).toEqual([
					{
						productId: createProduct.id,
					...
					input: [
						{
							...
							sku: 'SC01',
							price: 1399,
							translations: [{ languageCode: LanguageCode.en, name: 'Strawberry Cheesecake Pie' }],
						},
					],
				})
				await awaitRunningJobs(adminClient)
		...
		describe('channel handling', () => {
			...
			it('removing product variant from channel', async () => {
				...
				const { search: searchUngrouped } = await testProductsAdmin({ groupByProduct: false })
				expect(searchUngrouped.items.map(i => i.productVariantId)).toEqual(['T_2', 'T_3', 'T_4', 'T_10'])
			}, 10000)
			...
			// https://github.com/majel-ecommerce/majel/issues/896
		...
		describe('multiple language handling', () => {
			...
			describe('search products grouped by product and sorted by name ASC', () => {
				...
				// https://github.com/majel-ecommerce/majel/issues/1752
				// https://github.com/majel-ecommerce/majel/issues/1746
				it('fallbacks to default language en', async () => {
					const { search } = await searchInLanguage(LanguageCode.af)
		...
		// https://github.com/majel-ecommerce/majel/issues/1789
