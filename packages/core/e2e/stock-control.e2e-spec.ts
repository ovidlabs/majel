...
class TestOrderPlacedStrategy extends DefaultOrderPlacedStrategy {
	shouldSetAsPlaced(ctx: RequestContext, fromState: OrderState, toState: OrderState, order: Order): boolean {
		if ((order.customFields as any).test1557) {
			// This branch is used in testing https://github.com/majel-ecommerce/majel/issues/1557
...
describe('Stock control', () => {
	...
	describe('sales', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/1198
	...
	describe('saleable stock level', () => {
		...
		it('stockLevel uses DefaultStockDisplayStrategy', async () => {
			...
			expect(product?.variants.map(v => v.stockLevel)).toEqual(['OUT_OF_STOCK', 'IN_STOCK', 'IN_STOCK'])
		})
		...
		it('returns InsufficientStockError when tracking inventory & adding too many at once', async () => {
			...
			expect(addItemToOrder.errorCode).toBe(ErrorCode.INSUFFICIENT_STOCK_ERROR)
			expect(addItemToOrder.message).toBe('Only 3 items were added to the order due to insufficient stock')
		...
		it('returns InsufficientStockError for positive threshold', async () => {
			...
			expect(addItemToOrder.errorCode).toBe(ErrorCode.INSUFFICIENT_STOCK_ERROR)
			expect(addItemToOrder.message).toBe('Only 1 item was added to the order due to insufficient stock')
		...
		describe('edge cases', () => {
			...
			// https://github.com/majel-ecommerce/majel/issues/691
			...
			// https://github.com/majel-ecommerce/majel/issues/1273
			...
			// https://github.com/majel-ecommerce/majel/issues/1557
	...
	// https://github.com/majel-ecommerce/majel/issues/1028
	...
	// https://github.com/majel-ecommerce/majel/issues/1738
