...
import { CREATE_FULFILLMENT, CREATE_SHIPPING_METHOD, TRANSIT_FULFILLMENT } from './graphql/shared-definitions'
...
describe('Order fulfillments', () => {
	...
	const fulfillmentGuard: ErrorResultGuard<FulfillmentFragment> = createErrorResultGuard(input => !!input.id)
	...
	it('fulfillmentHandlers query', async () => {
		...
		expect(fulfillmentHandlers.map(h => h.code)).toEqual(['manual-fulfillment', 'test-fulfillment-handler'])
	})
