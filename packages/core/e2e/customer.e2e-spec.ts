...
import { AccountRegistrationEvent, EventBus, EventBusModule, mergeConfig, MajelPlugin } from '@majel/core'
...
describe('Customer resolver', () => {
	...
	describe('addresses', () => {
		...
		it('updateCustomerAddress allows only a single default address', async () => {
			...
			const result2 = await adminClient.query<Codegen.GetCustomerQuery, Codegen.GetCustomerQueryVariables>(
				GET_CUSTOMER,
				{
					id: firstCustomer.id,
				},
			...
			const result4 = await adminClient.query<Codegen.GetCustomerQuery, Codegen.GetCustomerQueryVariables>(
				GET_CUSTOMER,
				{
					id: firstCustomer.id,
				},
			...
			const result5 = await adminClient.query<Codegen.GetCustomerQuery, Codegen.GetCustomerQueryVariables>(
				GET_CUSTOMER,
				{
			...
			const result7 = await adminClient.query<Codegen.GetCustomerQuery, Codegen.GetCustomerQueryVariables>(
				GET_CUSTOMER,
				{
					id: firstCustomer.id,
				},
	...
	describe('update', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/1071
		...
		// https://github.com/majel-ecommerce/majel/issues/2449
	...
	describe('deletion', () => {
		...
		it(
			...
			assertThrowsWithMessage(
				() =>
					adminClient.query<Codegen.UpdateCustomerMutation, Codegen.UpdateCustomerMutationVariables>(
						UPDATE_CUSTOMER,
						{
							input: {
								id: thirdCustomer.id,
		...
		// https://github.com/majel-ecommerce/majel/issues/1960
