...
describe('Product prices', () => {
	...
	let multiPriceVariant: NonNullable<Codegen.CreateProductVariantsMutation['createProductVariants'][number]>
	...
	describe('ProductVariantPriceUpdateStrategy', () => {
		...
		it('syncing prices in other channels', async () => {
			...
			const { product: productDefaultChannel } = await adminClient.query(GetProductWithVariantsDocument, {
				id: multiPriceProduct.id,
			})
