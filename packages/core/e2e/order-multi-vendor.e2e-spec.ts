...
import { mergeConfig, OrderService } from '@majel/core'
...
describe('Multi-vendor orders', () => {
	...
	it('assign products to sellers', async () => {
		...
		const { assignProductsToChannel: result2 } = await adminClient.query(AssignProductsToChannelDocument, {
			input: {
	...
	it('order lines get split', async () => {
		...
		expect(order?.sellerOrders?.[0].lines.map(l => l.productVariant.name)).toEqual(['Laptop 13 inch 8GB'])
