import { mergeConfig } from '@majel/core'
...
describe('Transaction infrastructure', () => {
	...
	itIfDb(['postgres', 'mysql'])(
		...
		async () => {
			try {
				...
				expect(e.message).toMatch(/^Failed!|Query runner already released. Cannot run queries anymore.$/)
			}
	...
	it('non-failing mutation inside connection.withTransaction() wrapper with failing nested transactions and request context', async () => {
		...
		expect(verify.users.filter((u: any) => u.identifier.includes('testNestedTransactionsN'))).toHaveLength(
			1,
		)
	})
	...
	// Testing https://github.com/majel-ecommerce/majel/issues/520
	...
	// Testing https://github.com/majel-ecommerce/majel/issues/1107
