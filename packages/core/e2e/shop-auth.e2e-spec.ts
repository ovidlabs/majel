...
import { ErrorCode, RegisterCustomerInput } from '@majel/common/lib/generated-shop-types'
...
describe('Shop auth & accounts', () => {
	...
	describe('customer account creation with deferred password', () => {
		...
		it('register a new account without password', async () => {
			...
			expect(pick(customers.items[0], ['firstName', 'lastName', 'emailAddress', 'phoneNumber'])).toEqual(
				input,
			)
		})
	...
	// https://github.com/majel-ecommerce/majel/issues/1659
	...
	describe('updating emailAddress', () => {
		...
		it('return error result for bad token', async () => {
			...
			expect(updateCustomerEmailAddress.errorCode).toBe(ErrorCode.IDENTIFIER_CHANGE_TOKEN_INVALID_ERROR)
		})
...
function getEmailUpdateTokenPromise(): Promise<{
	...
	return new Promise(resolve => {
		sendEmailFn.mockImplementation((event: IdentifierChangeRequestEvent) => {
			resolve(
				pick(event.user.getNativeAuthenticationMethod(), ['identifierChangeToken', 'pendingIdentifier']),
			)
		})
	})
}
