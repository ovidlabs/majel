...
import { DefaultJobQueuePlugin, DefaultSearchPlugin, mergeConfig, UuidIdStrategy } from '@majel/core'
...
describe('Default search plugin with UUIDs', () => {
	...
	it('can filter by facetValueIds', async () => {
		...
		expect(result.search.items.map(i => i.productName)).toEqual(['Bonsai Tree', 'Orchid', 'Spiky Cactus'])
	})
