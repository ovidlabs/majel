...
import { Asset, CustomFields, mergeConfig, TransactionalConnection } from '@majel/core'
...
describe('Custom fields', () => {
	...
	describe('validation', () => {
					...
					mutation {
						updateProduct(
							input: {
								id: "T_1"
								translations: [
									{ id: "T_1", languageCode: en, customFields: { validateLocaleString: "servus" } }
								]
							}
						) {
							id
		...
		it(
			...
			assertThrowsWithMessage(async () => {
				await adminClient.query(gql`
					mutation {
						updateProduct(
							input: { id: "T_1", customFields: { validateDateTime: "2019-01-01T05:25:00.000Z" } }
						) {
							id
						}
					}
		...
		it(
			...
			assertThrowsWithMessage(async () => {
				await adminClient.query(gql`
					mutation {
						updateProduct(input: { id: "T_1", customFields: { intListWithValidation: [1, 2, 3] } }) {
							id
						}
					}
				`)
		...
		// https://github.com/majel-ecommerce/majel/issues/1000
		it(
		...
		// https://github.com/majel-ecommerce/majel/issues/1091
		...
		// https://github.com/majel-ecommerce/majel/issues/1953
		describe('validation of OrderLine custom fields', () => {
			it('addItemToOrder', async () => {
				try {
					const { addItemToOrder } = await shopClient.query(gql`
						mutation {
							addItemToOrder(productVariantId: 1, quantity: 1, customFields: { validateInt: 11 }) {
								... on Order {
									id
								}
							}
			...
			it('adjustOrderLine', async () => {
				try {
					const { adjustOrderLine } = await shopClient.query(gql`
						mutation {
							adjustOrderLine(orderLineId: "T_1", quantity: 1, customFields: { validateInt: 11 }) {
								... on Order {
									id
								}
							}
	...
	describe('public access', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/3049
	...
	describe('sort & filter', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/1581
