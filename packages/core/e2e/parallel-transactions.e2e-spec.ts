...
describe('Parallel transactions', () => {
	...
	it('does not fail on attempted deadlock', async () => {
		...
		expect(result).toEqual(Array.from({ length: CONCURRENCY_LIMIT }).map(() => ({ attemptDeadlock: true })))
	}, 100000)

	// A real-world error-case originally reported in https://github.com/majel-ecommerce/majel/issues/527
