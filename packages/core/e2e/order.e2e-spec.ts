...
describe('Orders resolver', () => {
	...
	describe('fulfillment', () => {
		...
		it('creates the first fulfillment', async () => {
			...
			expect(result.order!.fulfillments![0]!.lines.filter(l => l.orderLineId === lines[1].id).length).toBe(
				0,
			)
		})
	...
	describe('cancellation by orderId', () => {
		it('cancel from AddingItems state', async () => {
			const testOrder = await createTestOrder(adminClient, shopClient, customers[0].emailAddress, password)
			const { order } = await adminClient.query<Codegen.GetOrderQuery, Codegen.GetOrderQueryVariables>(
				GET_ORDER,
				{
	...
	describe('cancellation by OrderLine', () => {
		...
		beforeAll(async () => {
			const result = await createTestOrder(adminClient, shopClient, customers[0].emailAddress, password)
		...
		it('cannot cancel from AddingItems state', async () => {
			...
			expect(cancelOrder.message).toBe('Cannot cancel OrderLines from an Order in the "AddingItems" state')
		...
		it('returns error result if attempting to cancel already cancelled item', async () => {
			...
			expect(cancelOrder.message).toBe('The specified quantity is greater than the available OrderItems')
	...
	describe('refunds', () => {
		...
		// https://github.com/majel-ecommerce/majel/issues/873
		...
		// https://github.com/majel-ecommerce/majel/issues/2302
	...
	describe('order notes', () => {
		...
		beforeAll(async () => {
			const result = await createTestOrder(adminClient, shopClient, customers[2].emailAddress, password)
	...
	describe('multiple payments', () => {
		...
		beforeAll(async () => {
			const result = await createTestOrder(adminClient, shopClient, customers[1].emailAddress, password)
			orderId = result.orderId
		...
		it('adds another payment to make up order totalWithTax', async () => {
			...
			expect(
				omit(order.payments!.find(p => p.method === singleStageRefundablePaymentMethod.code)!, ['id']),
			).toEqual({
		...
		it('partial refunding of order with multiple payments', async () => {
			...
			expect(orderWithPayments?.payments!.sort(sortById)[0].refunds[0].total).toBe(PARTIAL_PAYMENT_AMOUNT)
		...
		// https://github.com/majel-ecommerce/majel/issues/847
	...
	// https://github.com/majel-ecommerce/majel/issues/2505
	...
	describe('issues', () => {
		// https://github.com/majel-ecommerce/majel/issues/639
		...
		// https://github.com/majel-ecommerce/majel/issues/603
		...
		// https://github.com/majel-ecommerce/majel/issues/716
		...
		// https://github.com/majel-ecommerce/majel/issues/868
		...
		// https://github.com/majel-ecommerce/majel/issues/1125
		...
		// https://github.com/majel-ecommerce/majel/issues/1508
		...
		// https://github.com/majel-ecommerce/majel/issues/2204
		...
		// https://github.com/majel-ecommerce/majel/issues/2191
