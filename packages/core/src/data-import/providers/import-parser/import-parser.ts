...
import { GlobalFlag, LanguageCode } from '@majel/common/lib/generated-types'
import { normalizeString } from '@majel/common/lib/normalize-string'
...
export class ImportParser {
	 ...
	 * returns a data structure which can then be used to populate Majel using the {@link FastImporterService}.
	 */
	async parseProducts(
		input: string | Stream,
	...
	private isTranslatable(baseKey: string): boolean {
		...
		if (parts.length === 2) {
			...
			const customFieldConfig = customFieldConfigs.find(config => config.name === parts[1])
			if (!customFieldConfig) {
				throw new InternalServerError(`Could not find custom field config for column header '${baseKey}'`)
			}
	...
	private parseProductFromRecord(
		...
		const translations = translationCodes.map(languageCode => {
			...
			const name = translatedFields.hasOwnProperty('name') ? parseString(translatedFields.name) : r.name
			let slug: string
...
function populateOptionGroupValues(currentRow: ParsedProductWithVariants) {
	for (const translation of currentRow.product.translations) {
		...
		currentRow.product.optionGroups.forEach((og, i) => {
			...
			if (!ogTranslation) {
				throw new InternalServerError(`No translation '${translation.languageCode}' for option groups'`)
			}
