 ...
 * @docsCategory migration
 */
export async function runMigrations(userConfig: Partial<MajelConfig>): Promise<string[]> {
 ...
 * @docsCategory migration
 */
export async function revertLastMigration(userConfig: Partial<MajelConfig>) {
	const config = await preBootstrapConfig(userConfig)
	const connection = await createConnection(createConnectionOptions(config))
	try {
...
function createConnectionOptions(userConfig: Partial<MajelConfig>): DataSourceOptions {
...
function log(message: string) {
	// If running from within the Majel CLI, we allow the CLI app
...
function isRunningFromMajelCli(): boolean {
	return process.env.MAJEL_RUNNING_IN_CLI != null
}
