...
import { JobState, LanguageCode } from '@majel/common/lib/generated-types'
	...
	private async saveVariants(ctx: MutableRequestContext, variants: ProductVariant[]) {
		...
		for (const variant of variants) {
			...
			for (const languageCode of availableLanguageCodes) {
				...
				channelIds.push(
					...clone.channels.filter(x => x.availableLanguageCodes.includes(languageCode)).map(x => x.id),
				)
				...
				for (const channel of variant.channels) {
					...
					const item = new SearchIndexItem({
						...
						productPreviewFocalPoint: product.featuredAsset ? product.featuredAsset.focalPoint : null,
						...
						collectionSlugs: collectionTranslations.map(c => c?.slug).filter(notNullOrUndefined) ?? [],
					})
