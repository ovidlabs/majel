...
import { Logger, RuntimeMajelConfig, MajelConfig } from '../config'
 ...
 * via the main Majel url.
 *
 * @example
 * ```ts
 * // Example usage in the `configuration` method of a MajelPlugin.
 * // Imagine that we have started a Node server on port 5678
 * // running some service which we want to access via the `/my-plugin/`
 * // route of the main Majel server.
 * \@MajelPlugin({
 *   configuration: (config: Required<MajelConfig>) => {
...
export interface ProxyOptions {
	...
	/**
	 * @description
	 * The route of the Majel server which will act as the proxy url.
	 */
	route: string
	/**
	 * @description
