...
/**
 * @description
 * Populates the Majel server with some initial data and (optionally) product data from
 ...
 * import { populate } from '\@majel/core/cli';
 * import { config } from './majel-config.ts'
...
export async function populate<T extends INestApplicationContext>(
	...
	channelOrToken?: string | import('@majel/core').Channel,
): Promise<T> {
	const app = await bootstrapFn()
	if (!app) {
		throw new Error('Could not bootstrap the Majel app')
	}
	let channel: import('@majel/core').Channel | undefined
	const { ChannelService, Channel, Logger } = await import('@majel/core')
	...
	const initialData: import('@majel/core').InitialData =
	...
	if (productsCsvPath) {
		...
		if (importResult.errors && importResult.errors.length) {
			const errorFile = path.join(process.cwd(), 'majel-import-error.log')
			Logger.error(
...
export async function populateInitialData(
	...
	initialData: import('@majel/core').InitialData,
	channel?: import('@majel/core').Channel,
) {
	const { Populator, Logger } = await import('@majel/core')
...
export async function importProductsFromCsv(
	app: INestApplicationContext,
	productsCsvPath: string,
	languageCode: import('@majel/core').LanguageCode,
	channel?: import('@majel/core').Channel,
): Promise<import('@majel/core').ImportProgress> {
	const { Importer, RequestContextService } = await import('@majel/core')
