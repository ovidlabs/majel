...
export async function createSelfRefreshingCache<V, RefreshArgs extends any[]>(
	...
	const refreshValue = (resetMemoCache = true, args: RefreshArgs): Promise<V> => {
		return refresh
			...
			.catch((err: any) => {
				...
				Logger.error(`Failed to update SelfRefreshingCache "${name}": ${message}`, undefined, err.stack)
				return value
			})
	}
	...
	return {
		value: (...args) =>
			getValue(
				!args.length || (args.length === 1 && args[0] === undefined) ? undefined : (args as RefreshArgs),
			),
