...
export class AuthGuard implements CanActivate {
	...
	private async setActiveChannel(requestContext: RequestContext, session?: CachedSession): Promise<boolean> {
		if (!session) {
			return false
		}
				...
				if (customer) {
					try {
						...
						if (isDuplicateError) {
							...
							// See https://github.com/majel-ecommerce/majel/issues/834
						} else {
							throw e
						}
					}
