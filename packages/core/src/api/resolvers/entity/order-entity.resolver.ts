...
import { HistoryEntryListOptions, OrderHistoryArgs, SortOrder } from '@majel/common/lib/generated-types'
...
export class OrderEntityResolver {
	...
	async promotions(@Ctx() ctx: RequestContext, @Parent() order: Order) {
		...
		if (
			order.promotions &&
			(order.promotions.length === 0 || (order.promotions.length > 0 && order.promotions[0].translations))
		) {
