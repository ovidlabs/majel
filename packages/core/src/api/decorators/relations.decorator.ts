...
export type RelationPaths<T extends MajelEntity> = Array<EntityRelationPaths<T>>

export type FieldsDecoratorConfig<T extends MajelEntity> =
	| Type<T>
	| {
			entity: Type<T>
			depth?: number
 ...
 * @since 1.6.0
 */
export const Relations: <T extends MajelEntity>(data: FieldsDecoratorConfig<T>) => ParameterDecorator =
	...
	for (const [property, value] of Object.entries(fields)) {
		if (property === 'customFields') {
			...
			if (customFieldEntity) {
				if (depth < maxDepth) {
					...
					const subPaths = getRelationPaths(
						value,
						customFieldEntity as Type<MajelEntity>,
						maxDepth,
						depth,
					)
					depth--
			...
			if (relationMetadata) {
				...
				if (depth < maxDepth) {
					depth++
					const subPaths = getRelationPaths(value, relatedEntity as Type<MajelEntity>, maxDepth, depth)
					depth--
					for (const subPath of subPaths) {
						relationPaths.push([property, subPath].join('.'))
					}
