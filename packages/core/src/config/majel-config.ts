...
import { MajelLogger } from './logger/majel-logger'
...
/**
 * @description
 * The ApiOptions define how the Majel GraphQL APIs are exposed, as well as allowing the API layer
...
export interface ApiOptions {
	...
	/**
	 * @description
	 * Which port the Majel server should listen on.
	 *
	 * @default 3000
	 */
	port: number
	 ...
	 * allow the extension of the Apollo Server, which is the underlying GraphQL server used by Majel.
	 *
...
export interface AuthOptions {
	 ...
	 *
	 * Note that if the bearer method is used, Majel will automatically expose the configured
	 * `authTokenHeaderKey` in the server's CORS configuration (adding `Access-Control-Expose-Headers: majel-auth-token`
	 * by default).
	 *
...
export interface OrderOptions {
	 ...
	 * adding or removing an item) will require Majel to loop through all million items
	 ...
	 *
	 * Attempting to exceed this limit will cause Majel to throw a `OrderLimitError`.
	 *
	 * @default 999
	 */
	 ...
	 * integrating with existing systems. By default, Majel will generate a 16-character
	 * alphanumeric string.
	 *
...
/**
 * @description
 * Defines payment-related options in the {@link MajelConfig}.
 *
...
export interface EntityOptions {
	 ...
	 * on an existing Majel database will lead to problems with broken foreign-key
...
export interface SystemOptions {
	 ...
	 * that any critical systems which the Majel server depends on are also healthy.
	 *
 ...
 * [`MajelConfig`](https://github.com/majel-ecommerce/majel/blob/master/server/src/config/majel-config.ts) interface.
 *
 * @docsCategory configuration
 * */
export interface MajelConfig {
	/**
	 * @description
	...
	/**
	 * @description
	 * Provide a logging service which implements the {@link MajelLogger} interface.
...
/**
 * @description
 * This interface represents the MajelConfig object available at run-time, i.e. the user-supplied
 * config values have been merged with the {@link defaultConfig} values.
 *
 * @docsCategory configuration
 */
export interface RuntimeMajelConfig extends Required<MajelConfig> {
...
export type PartialMajelConfig = DeepPartialSimple<MajelConfig>
