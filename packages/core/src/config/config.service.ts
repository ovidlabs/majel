...
import { Logger, MajelLogger } from './logger/majel-logger'
import {
	ApiOptions,
	AssetOptions,
	AuthOptions,
...
@Injectable()
export class ConfigService implements MajelConfig {
	private activeConfig: RuntimeMajelConfig
			...
			for (const entity of this.dbConnectionOptions.entities) {
				if (typeof entity === 'function' && !definedCustomFields[entity.name]) {
					...
					const isTranslationEntity =
						...
						metadataArgsStorage.filterColumns(entity).find(c => c.propertyName === 'languageCode')
