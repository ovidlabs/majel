...
import { Logger } from './majel-logger'
...
/**
 * A custom logger for TypeORM which delegates to the Majel Logger service.
 */
export class TypeOrmLogger implements TypeOrmLoggerInterface {
	...
	private formatQueryWithParams(query: string, parameters?: any[]) {
		return (
			query + (parameters?.length ? ' -- PARAMETERS: ' + this.stringifyParams(parameters).toString() : '')
		)
	}
