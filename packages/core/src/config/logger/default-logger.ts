...
import { Logger, LogLevel, MajelLogger } from './majel-logger'

const DEFAULT_CONTEXT = 'Majel Server'

/**
 * @description
 * The default logger, which logs to the console (stdout) with optional timestamps. Since this logger is part of the
 * default Majel configuration, you do not need to specify it explicitly in your server config. You would only need
 * to specify it if you wish to change the log level (which defaults to `LogLevel.Info`) or remove the timestamp.
 *
 * @example
 * ```ts
 * import { DefaultLogger, LogLevel, MajelConfig } from '\@majel/core';
 *
 * export config: MajelConfig = {
 *     // ...
 ...
 * @docsCategory Logger
 */
export class DefaultLogger implements MajelLogger {
	/** @internal */
