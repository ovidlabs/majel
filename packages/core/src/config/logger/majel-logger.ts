...
/**
 * @description
 * The MajelLogger interface defines the shape of a logger service which may be provided in
 * the config.
 *
 * @docsCategory Logger
 */
export interface MajelLogger {
...
/**
 * @description
 * The Logger is responsible for all logging in a Majel application.
 *
 ...
 *
 * Logger.info(`Some log message`, 'My Majel Plugin');
 * ```
 *
 * The actual implementation - where the logs are written to - is defined by the {@link MajelLogger}
 * instance configured in the {@link MajelConfig}. By default, the {@link DefaultLogger} is used, which
 * logs to the console.
 *
 * ## Implementing a custom logger
 *
 * A custom logger can be passed to the `logger` config option by creating a class which implements the
 * {@link MajelLogger} interface. For example, here is how you might go about implementing a logger which
 * logs to a file:
 *
 * @example
 * ```ts
 ...
 * export class SimpleFileLogger implements MajelLogger {
...
export class Logger implements LoggerService {
	...
	private static _logger: MajelLogger

	static get logger(): MajelLogger {
	...
	/** @internal */
	static useLogger(logger: MajelLogger) {
