...
/**
 * @description
 * Used to configure a customized instance of the default {@link OrderProcess} that ships with Majel.
 * Using this function allows you to turn off certain checks and constraints that are enabled by default.
 *
 * ```ts
 * import { configureDefaultOrderProcess, MajelConfig } from '\@majel/core';
 *
...
export function configureDefaultOrderProcess(options: DefaultOrderProcessOptions) {
	...
	const orderProcess: OrderProcess<OrderState> = {
		...
		async onTransitionStart(fromState, toState, { ctx, order }) {
			...
			if (options.checkAdditionalPaymentsAmount !== false && fromState === 'ArrangingAdditionalPayment') {
...
/**
 * @description
 * This is the built-in {@link OrderProcess} that ships with Majel. A customized version of this process
