...
export class TransactionalConnection {
	 ...
	 * is found.
	 */
	async getEntityOrThrow<T extends MajelEntity>(
		ctx: RequestContext,
		entityType: Type<T>,
		id: ID,
	...
	private async getEntityOrThrowInternal<T extends MajelEntity>(
		ctx: RequestContext,
		entityType: Type<T>,
		id: ID,
	 ...
	 * the given Channel.
	 */
	findOneInChannel<T extends ChannelAware & MajelEntity>(
		ctx: RequestContext,
		entity: Type<T>,
		id: ID,
		channelId: ID,
	 ...
	 * the given Channel.
	 */
	findByIdsInChannel<T extends ChannelAware | MajelEntity>(
		ctx: RequestContext,
		entity: Type<T>,
		ids: ID[],
		channelId: ID,
		...
		if (Array.isArray(options.relations) && options.relations.length > 0) {
			const joinedRelations = joinTreeRelationsDynamically(
				qb as SelectQueryBuilder<MajelEntity>,
				entity,
				options.relations,
			)
			// Remove any relations which are related to the 'collection' tree, as these are handled separately
