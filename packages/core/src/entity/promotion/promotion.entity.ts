import { Adjustment, AdjustmentType, ConfigurableOperation } from '@majel/common/lib/generated-types'
	...
	implements ChannelAware, SoftDeletable, HasCustomFields, Translatable
{
	...
	constructor(
		...
		const actions = (input && input.promotionActions) || getConfig().promotionOptions.promotionActions || []
		...
		for (const action of this.actions) {
			...
			if (promotionAction instanceof PromotionItemAction) {
				if (this.isOrderItemArg(args)) {
					...
					amount += roundMoney(await promotionAction.execute(ctx, orderLine, action.args, state, this))
				}
