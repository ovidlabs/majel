import { Adjustment, AdjustmentType, Discount, TaxLine } from '@majel/common/lib/generated-types'
 ...
 */
@Entity()
export class ShippingLine extends MajelEntity {
		...
		return (
			this.adjustments?.map(adjustment => {
				const amount = roundMoney(
					this.listPriceIncludesTax ? netPriceOf(adjustment.amount, this.taxRate) : adjustment.amount,
				)
				const amountWithTax = roundMoney(
					this.listPriceIncludesTax ? adjustment.amount : grossPriceOf(adjustment.amount, this.taxRate),
				)
				return {
