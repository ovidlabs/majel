...
describe('EventBus', () => {
	...
	describe('filter()', () => {
		it('single handler is called once', async () => {
			const handler = vi.fn()
			const event = new TestEvent('foo')
			eventBus.filter(majelEvent => majelEvent instanceof TestEvent).subscribe(handler)
		...
		it('multiple handler are called', async () => {
			...
			const handler3 = vi.fn()
			const event = new TestEvent('foo')
			eventBus.filter(majelEvent => majelEvent instanceof TestEvent).subscribe(handler1)
			eventBus.filter(majelEvent => majelEvent instanceof TestEvent).subscribe(handler2)
			eventBus.filter(majelEvent => majelEvent instanceof TestEvent).subscribe(handler3)
		...
		it('filter() returns a subscription', async () => {
			...
			const subscription = eventBus
				.filter(majelEvent => majelEvent instanceof TestEvent)
				.subscribe(handler)
	...
	describe('blocking event handlers', () => {
		it('calls the handler function', async () => {
			const event = new TestEvent('foo')
			const spy = vi.fn((e: MajelEvent) => undefined)
		...
		it('throws when attempting to register with a duplicate id', () => {
			...
			}).toThrowError('A handler with the id "test-handler" is already registered for the event TestEvent')
		})

		it('calls multiple handler functions', async () => {
			const event = new TestEvent('foo')
			const spy1 = vi.fn((e: MajelEvent) => undefined)
			const spy2 = vi.fn((e: MajelEvent) => undefined)
			eventBus.registerBlockingEventHandler({
				handler: e => spy1(e),
				id: 'test-handler1',
				event: TestEvent,
		...
		it('throws if there is a cycle in before ordering', () => {
			...
			).toThrowError('Circular dependency detected between event handlers test-handler1 and test-handler2')
		})
...
class TestEvent extends MajelEvent {
...
class OtherTestEvent extends MajelEvent {
	constructor(public payload: string) {
		super()
	}
}
