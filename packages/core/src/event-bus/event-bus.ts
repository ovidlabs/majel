 ...
 * @docsCategory events
 */
export type BlockingEventHandlerOptions<T extends MajelEvent> = {
	/**
	 * @description
 ...
 *
 * Events are published whenever certain actions take place within the Majel server, for example:
 *
 ...
 * import { EventBus, PluginCommonModule, MajelPlugin } from '\@majel/core';
...
export class EventBus implements OnModuleDestroy {
	private eventStream = new Subject<MajelEvent>()
	private destroy$ = new Subject<void>()
	private blockingEventHandlers = new Map<Type<MajelEvent>, Array<BlockingEventHandlerOptions<any>>>()
	 ...
	 * ```
	 */
	async publish<T extends MajelEvent>(event: T): Promise<void> {
	 ...
	 */
	ofType<T extends MajelEvent>(type: Type<T>): Observable<T> {
	 ...
	 * data related to the event.
	 */
	filter<T extends MajelEvent>(predicate: (event: MajelEvent) => boolean): Observable<T> {
		return this.eventStream.asObservable().pipe(
			takeUntil(this.destroy$),
	 ...
	 * @since 2.2.0
	 */
	registerBlockingEventHandler<T extends MajelEvent>(handlerOptions: BlockingEventHandlerOptions<T>) {
	...
	private async executeBlockingEventHandlers<T extends MajelEvent>(event: T): Promise<void> {
	...
	private orderEventHandlers<T extends MajelEvent>(
	 ...
	 *
	 * * https://github.com/majel-ecommerce/majel/issues/520
	 * * https://github.com/majel-ecommerce/majel/issues/1107
	 */
	private async awaitActiveTransactions<T extends MajelEvent>(event: T): Promise<T | undefined> {
