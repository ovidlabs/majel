...
import { ApplyCouponCodeResult } from '@majel/common/lib/generated-shop-types'
import {
...
export class PromotionService {
	...
	async updatePromotion(
		...
		const hasConditions = input.conditions ? input.conditions.length > 0 : promotion.conditions.length > 0
	...
	async addPromotionsToOrder(ctx: RequestContext, order: Order): Promise<Order> {
		const allPromotionIds = order.discounts.map(a => AdjustmentSource.decodeSourceId(a.adjustmentSource).id)
