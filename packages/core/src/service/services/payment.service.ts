...
import { ManualPaymentInput, RefundOrderInput } from '@majel/common/lib/generated-types'
...
export class PaymentService {
	...
	async createRefund(
		...
		if (input.amount) {
			const paymentToRefund = orderWithRefunds.payments.find(p => idsAreEqual(p.id, selectedPayment.id))
			if (!paymentToRefund) {
