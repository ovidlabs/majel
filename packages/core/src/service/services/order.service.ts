...
} from '@majel/common/lib/generated-shop-types'
import {
...
export class OrderService {
	...
	async addItemToOrder(
		...
		const existingQuantityInOtherLines = summate(
			order.lines.filter(
				l =>
					idsAreEqual(l.productVariantId, productVariantId) && !idsAreEqual(l.id, existingOrderLine?.id),
			),
			'quantity',
		)
	...
	async adjustOrderLine(
		...
		const existingQuantityInOtherLines = summate(
			order.lines.filter(
				l =>
					idsAreEqual(l.productVariantId, orderLine.productVariantId) && !idsAreEqual(l.id, orderLineId),
			),
			'quantity',
		)
		const correctedQuantity = await this.orderModifier.constrainQuantityToSaleable(
	...
	async removeCouponCode(ctx: RequestContext, orderId: ID, couponCode: string) {
		...
		if (order.couponCodes.includes(couponCode)) {
			...
			const affectedOrderLines = order.lines.filter(
				line =>
					line.adjustments.filter(a => a.type === AdjustmentType.DISTRIBUTED_ORDER_PROMOTION).length,
			)
	...
	async addCustomerToOrder(
		...
		const order =
			orderIdOrOrder instanceof Order ? orderIdOrOrder : await this.getOrderOrThrow(ctx, orderIdOrOrder)
	...
	async deleteOrder(ctx: RequestContext, orderOrId: ID | Order) {
		...
		// reference in order to avoid a foreign key error. See https://github.com/majel-ecommerce/majel/issues/1454
	...
	async mergeOrders(
		...
		if (guestOrder && guestOrder.customer) {
			...
			// so we do not want to merge at all. See https://github.com/majel-ecommerce/majel/issues/263
