...
import { PaymentMethodQuote } from '@majel/common/lib/generated-shop-types'
import {
...
export class PaymentMethodService {
	...
	async update(ctx: RequestContext, input: UpdatePaymentMethodInput): Promise<PaymentMethod> {
		...
		await this.customFieldRelationService.updateRelations(ctx, PaymentMethod, input, updatedPaymentMethod)
	...
	async delete(ctx: RequestContext, paymentMethodId: ID, force: boolean = false): Promise<DeletionResponse> {
		...
		if (ctx.channel.code === DEFAULT_CHANNEL_CODE) {
			...
			await this.eventBus.publish(new PaymentMethodEvent(ctx, paymentMethod, 'deleted', paymentMethodId))
			return {
				result: DeletionResult.DELETED,
			}
		}
	...
	async assignPaymentMethodsToChannel(
		...
		for (const paymentMethodId of input.paymentMethodIds) {
			...
			await this.channelService.assignToChannels(ctx, PaymentMethod, paymentMethodId, [input.channelId])
		}
		return this.connection
	...
	async removePaymentMethodsFromChannel(
		...
		for (const paymentMethodId of input.paymentMethodIds) {
			const paymentMethod = await this.connection.getEntityOrThrow(ctx, PaymentMethod, paymentMethodId)
			await this.channelService.removeFromChannels(ctx, PaymentMethod, paymentMethodId, [input.channelId])
		}
		return this.connection
			.findByIdsInChannel(ctx, PaymentMethod, input.paymentMethodIds, ctx.channelId, {})
			.then(methods => methods.map(method => this.translator.translate(method, ctx)))
