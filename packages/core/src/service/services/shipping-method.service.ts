...
import { ID, PaginatedList } from '@majel/common/lib/shared-types'
...
export class ShippingMethodService {
		...
		if (this.configService.shippingOptions.fulfillmentHandlers.length === 0) {
			throw new Error(
				'No FulfillmentHandlers were found.' +
					' Please ensure the MajelConfig.shippingOptions.fulfillmentHandlers array contains at least one FulfillmentHandler.',
			)
		}
	...
	async create(ctx: RequestContext, input: CreateShippingMethodInput): Promise<Translated<ShippingMethod>> {
		...
		const newShippingMethod = await this.connection.getRepository(ctx, ShippingMethod).save(shippingMethod)
		...
		await this.eventBus.publish(new ShippingMethodEvent(ctx, shippingMethodWithRelations, 'created', input))
	...
	async update(ctx: RequestContext, input: UpdateShippingMethodInput): Promise<Translated<ShippingMethod>> {
		...
		await this.customFieldRelationService.updateRelations(ctx, ShippingMethod, input, updatedShippingMethod)
		await this.connection.getRepository(ctx, ShippingMethod).save(updatedShippingMethod, { reload: false })
	...
	async assignShippingMethodsToChannel(
		...
		for (const shippingMethodId of input.shippingMethodIds) {
			...
			await this.channelService.assignToChannels(ctx, ShippingMethod, shippingMethodId, [input.channelId])
		}
	...
	async removeShippingMethodsFromChannel(
		...
		for (const shippingMethodId of input.shippingMethodIds) {
			const shippingMethod = await this.connection.getEntityOrThrow(ctx, ShippingMethod, shippingMethodId)
	...
	getShippingEligibilityCheckers(ctx: RequestContext): ConfigurableOperationDefinition[] {
		return this.configArgService.getDefinitions('ShippingEligibilityChecker').map(x => x.toGraphQlType(ctx))
	}
