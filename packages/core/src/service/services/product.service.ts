...
export class ProductService {
	...
	async assignProductsToChannel(
		...
		await this.productVariantService.assignProductVariantsToChannel(ctx, {
			productVariantIds: ([] as ID[]).concat(...productsWithVariants.map(p => p.variants.map(v => v.id))),
	...
	async addOptionGroupToProduct(
		...
		await this.connection.getRepository(ctx, Product).save(product, { reload: false })
		await this.eventBus.publish(new ProductOptionGroupChangeEvent(ctx, product, optionGroupId, 'assigned'))
	...
	async removeOptionGroupFromProduct(
		...
		await this.eventBus.publish(new ProductOptionGroupChangeEvent(ctx, product, optionGroupId, 'removed'))
		return assertFound(this.findOne(ctx, productId))
	}
