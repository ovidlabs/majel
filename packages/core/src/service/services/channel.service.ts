...
import { DEFAULT_CHANNEL_CODE } from '@majel/common/lib/shared-constants'
import { ID, PaginatedList, Type } from '@majel/common/lib/shared-types'
...
export class ChannelService {
	 ...
	 */
	async assignToCurrentChannel<T extends ChannelAware & MajelEntity>(
		entity: T,
		ctx: RequestContext,
	): Promise<T> {
	 ...
	 * @private
	 */
	private async getAssignedEntityChannels<T extends ChannelAware & MajelEntity>(
		ctx: RequestContext,
		entityType: Type<T>,
		entityId: T['id'],
	 ...
	 */
	async assignToChannels<T extends ChannelAware & MajelEntity>(
		ctx: RequestContext,
		entityType: Type<T>,
		entityId: ID,
		channelIds: ID[],
		...
		const relations = []
		// This is a work-around for https://github.com/majel-ecommerce/majel/issues/1391
	 ...
	 */
	async removeFromChannels<T extends ChannelAware & MajelEntity>(
		ctx: RequestContext,
		entityType: Type<T>,
		entityId: ID,
		channelIds: ID[],
	...
	async create(
		...
		if (input.defaultTaxZoneId) {
			channel.defaultTaxZone = await this.connection.getEntityOrThrow(ctx, Zone, input.defaultTaxZoneId)
		}
	 ...
	 */
	public isChannelAware(entity: MajelEntity): entity is MajelEntity & ChannelAware {
