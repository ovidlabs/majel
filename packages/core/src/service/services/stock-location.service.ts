...
export class StockLocationService {
	...
	async update(ctx: RequestContext, input: UpdateStockLocationInput): Promise<StockLocation> {
		...
		await this.customFieldRelationService.updateRelations(ctx, StockLocation, input, updatedStockLocation)
	...
	async assignStockLocationsToChannel(
		...
		for (const stockLocationId of input.stockLocationIds) {
			...
			await this.channelService.assignToChannels(ctx, StockLocation, stockLocationId, [input.channelId])
		}
		return this.connection.findByIdsInChannel(ctx, StockLocation, input.stockLocationIds, ctx.channelId, {})
	}
	...
	async removeStockLocationsFromChannel(
		...
		for (const stockLocationId of input.stockLocationIds) {
			...
			await this.channelService.removeFromChannels(ctx, StockLocation, stockLocationId, [input.channelId])
		}
		return this.connection.findByIdsInChannel(ctx, StockLocation, input.stockLocationIds, ctx.channelId, {})
	}
