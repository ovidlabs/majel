...
export class CollectionService implements OnModuleInit {
	...
	async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
		...
		for (const coll of [...descendants.reverse(), collection]) {
			...
			await this.eventBus.publish(new CollectionModificationEvent(ctx, deletedColl, affectedVariantIds))
		}
	 ...
	 * variants (because other attributes of collection can be changed https://github.com/majel-ecommerce/majel/issues/1015)
	 */
