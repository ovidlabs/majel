...
import { Channel, Order, OrderLine, ProductOptionGroup, ProductVariantPrice, TaxCategory } from '../../entity'
...
export class ProductVariantService {
	...
	async createOrUpdateProductVariantPrice(
		...
		if (!targetPrice) {
			...
			const updatedPrice = await this.connection.getRepository(ctx, ProductVariantPrice).save(targetPrice)
		...
		if (uniqueAdditionalPricesToUpdate.length) {
			...
			await this.eventBus.publish(new ProductVariantPriceEvent(ctx, updatedAdditionalPrices, 'updated'))
		}
		return targetPrice
	}
	...
	async removeProductVariantsFromChannel(
		...
		for (const variant of variants) {
			...
			if (!productChannelsFromVariants.find(c => c.id === input.channelId)) {
				await this.channelService.removeFromChannels(ctx, Product, variant.productId, [input.channelId])
			}
		}
		const result = await this.findByIds(
			ctx,
		...
		for (const variant of variants) {
			await this.eventBus.publish(new ProductVariantChannelEvent(ctx, variant, input.channelId, 'removed'))
		}
		return result
	}
	...
	private async validateVariantOptionIds(ctx: RequestContext, productId: ID, optionIds: ID[] = []) {
		...
		// https://github.com/majel-ecommerce/majel/issues/328
		const optionGroups = (
