 ...
 */
export type ExtendedListQueryOptions<T extends MajelEntity> = {
	relations?: string[]
	channelId?: ID
 ...
 * }
 *
 * # Generated at run-time by Majel
 ...
 * }
 * ```
 * When Majel bootstraps, it will find the `BlogPostListOptions` input and, because it is used in a query
...
export class ListQueryBuilder implements OnApplicationBootstrap {
	 ...
	 */
	filterObjectHasProperty<FP extends FilterParameter<MajelEntity>>(
	 ...
	 * method.
	 */
	private getMinimumRequiredRelations<T extends MajelEntity>(
	 ...
	 */
	private normalizeCustomPropertyMap<T extends MajelEntity>(
		customPropertyMap: { [name: string]: string },
		options: ListQueryOptions<any>,
	 ...
	 */
	private joinCalculatedColumnRelations<T extends MajelEntity>(
	 ...
	 */
	private applyTranslationConditions<T extends MajelEntity>(
		qb: SelectQueryBuilder<any>,
		entity: Type<T>,
	...
	private isRelationAlreadyJoined<T extends MajelEntity>(qb: SelectQueryBuilder<T>, alias: string): boolean {
