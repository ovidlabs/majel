...
import { getGraphQlInputName } from '@majel/common/lib/shared-utils'
...
export class CustomFieldRelationService {
	 ...
	 * database.
	 */
	async updateRelations<T extends HasCustomFields & MajelEntity>(
		ctx: RequestContext,
		entityType: Type<T>,
		...
		if (input.customFields) {
			...
			for (const field of relationCustomFields) {
				...
				if (idOrIds !== undefined) {
					let relations: MajelEntity | MajelEntity[] | undefined | null
					if (idOrIds === null) {
