...
import { getGraphQlInputName, summate } from '@majel/common/lib/shared-utils'
...
export class OrderModifier {
	 ...
	 * OrderLines into account when constraining the quantity. See https://github.com/majel-ecommerce/majel/issues/2702
	 * for more on this.
	 */
	...
	private async customFieldsAreEqual(
		...
		for (const def of customFieldDefs) {
			...
			if (def.type !== 'relation' && existingValue !== undefined) {
				const valuesMatch = JSON.stringify(inputCustomFields?.[key]) === JSON.stringify(existingValue)
				...
				if (inputValue) {
					const customFieldNotEqual = def.list
						? JSON.stringify((inputValue as ID[]).sort()) !==
							JSON.stringify(existingRelation?.map((relation: MajelEntity) => relation.id).sort())
