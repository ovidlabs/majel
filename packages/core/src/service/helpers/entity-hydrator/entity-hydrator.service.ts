 ...
 * ```ts
 * import { Injectable } from '\@nestjs/common';
 * import { ID, RequestContext, EntityHydrator, ProductVariantService } from '\@majel/core';
 *
 * \@Injectable()
...
export class EntityHydrator {
	 ...
	 * @since 1.3.0
	 */
	async hydrate<Entity extends MajelEntity>(
		ctx: RequestContext,
		target: Entity,
	...
	private assignSettableProperties<Entity extends MajelEntity>(target: Entity, source: Entity) {
	 ...
	 */
	private getMissingRelations<Entity extends MajelEntity>(target: Entity, options: HydrateOptions<Entity>) {
	...
	private getRequiredProductVariantRelations<Entity extends MajelEntity>(
		target: Entity,
	...
	private getRelationEntityAtPath(
		entity: MajelEntity,
		path: string[],
	): MajelEntity | MajelEntity[] | undefined {
		let isArrayResult = false
		const result: MajelEntity[] = []
	...
	private getRelationEntityTypeAtPath(entity: MajelEntity, path: string): Type<MajelEntity> {
		...
		if (!targetMetadata) {
			throw new InternalServerError(`Cannot find entity metadata for entity "${entity.constructor.name}"`)
		}
		...
		return currentMetadata.target as Type<MajelEntity>
	}

	private isTranslatable<T extends MajelEntity>(input: T | T[] | undefined): boolean {
