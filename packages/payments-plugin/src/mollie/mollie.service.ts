...
import { OrderStateMachine } from '@majel/core/'
import { totalCoveredByPayments } from '@majel/core/dist/service/helpers/utils/order-utils'
...
export class MollieService {
	...
	async createPaymentIntent(
		...
		if (!redirectUrl) {
			...
			fallbackRedirect = fallbackRedirect.endsWith('/') ? fallbackRedirect.slice(0, -1) : fallbackRedirect
		...
		const majelHost = this.options.majelHost.endsWith('/')
			? this.options.majelHost.slice(0, -1)
			: this.options.majelHost // remove appending slash
		...
		const orderInput: CreateParameters = {
			...
			redirectUrl,
			webhookUrl: `${majelHost}/payments/mollie/${ctx.channel.token}/${paymentMethod.id}`,
			billingAddress,
	...
	/**
	 * Update Majel payments and order status based on the incoming Mollie order
	 */
	async handleMollieStatusUpdate(
		...
		// Any other combination of Mollie status and Majel status indicates something is wrong.
		throw Error(
	...
	/**
	 * Update an existing Mollie order based on the given Majel order.
	 */
	...
	/**
	 * Delete all order lines of current Mollie order, and create new ones based on the new Majel order lines
	 */
	private async updateMollieOrderLines(
		mollieClient: ExtendedMollieClient,
		existingMollieOrder: MollieOrder,
		/**
		 * These are the new order lines based on the Majel order
		 */
