...
import { MajelStripeClient } from './stripe-client'
...
export class StripeService {
	 ...
	 */
	async getStripeClient(ctx: RequestContext, order: Order): Promise<MajelStripeClient> {
		...
		return new MajelStripeClient(apiKey, webhookSecret)
	}
	...
	private async getStripeCustomerId(ctx: RequestContext, activeOrder: Order): Promise<string | undefined> {
		...
		if (stripeCustomers.data.length > 0) {
			...
			const newStripeCustomer = await stripe.customers.create({
				...
				...(additionalParams ?? {}),
				...(additionalParams?.metadata ? { metadata: sanitizeMetadata(additionalParams.metadata) } : {}),
			})
