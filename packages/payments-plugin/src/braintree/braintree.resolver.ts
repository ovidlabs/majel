...
export class BraintreeResolver {
	...
	async generateBraintreeClientToken(
		...
		if (!sessionOrder) {
			throw new InternalServerError('Cannot generate Braintree clientToken as there is no active Order.')
		}
		...
		if (order) {
			...
			try {
				...
				if (result.success === true) {
					...
					throw new InternalServerError(`Could not generate Braintree clientToken: ${result.message}`)
				}
			} catch (e: any) {
				Logger.error(
