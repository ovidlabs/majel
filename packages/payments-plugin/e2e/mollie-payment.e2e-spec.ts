...
import { SETTLE_PAYMENT } from '@majel/core/e2e/graphql/shared-definitions'
import {
...
describe('Mollie payments', () => {
	beforeAll(async () => {
		const devConfig = mergeConfig(testConfig(), {
			plugins: [MolliePlugin.init({ majelHost: mockData.host })],
		})
	...
	describe('Payment intent creation', () => {
		...
		it('Should recreate all order lines in Mollie', async () => {
			...
			nock('https://api.mollie.com/').get('/v2/orders/ord_mockId').reply(200, mockData.mollieOrderResponse)
		...
		it('Transitions to PaymentSettled for orders with a total of $0', async () => {
			...
			const { createMolliePaymentIntent: intent } = await shopClient.query(CREATE_MOLLIE_PAYMENT_INTENT, {
				input: {
					paymentMethodCode: mockData.methodCode,
