import { JobListOptions, JobState } from '@majel/common/lib/generated-types'
...
const QUEUE_NAME = 'majel-job-queue'
...
export class BullMQJobQueueStrategy implements InspectableJobQueueStrategy {
	...
	private readonly CANCELLED_JOB_LIST_NAME = 'majel:cancelled-jobs'
	...
	async init(injector: Injector): Promise<void> {
		...
		this.workerProcessor = async bullJob => {
			...
			if (processFn) {
				const job = await this.createMajelJob(bullJob)
	...
	async add<Data extends JobData<Data> = object>(job: Job<Data>): Promise<Job<Data>> {
		...
		return this.createMajelJob(bullJob)
	}
	...
	async findMany(options?: JobListOptions): Promise<PaginatedList<Job>> {
		...
		return {
			items: await Promise.all(items.map(bullJob => this.createMajelJob(bullJob))),
			totalItems,
		}
	}
	...
	async findManyById(ids: ID[]): Promise<Job[]> {
		...
		return Promise.all(bullJobs.filter(notNullOrUndefined).map(j => this.createMajelJob(j)))
	}
	...
	private async createMajelJob(bullJob: Bull.Job): Promise<Job> {
