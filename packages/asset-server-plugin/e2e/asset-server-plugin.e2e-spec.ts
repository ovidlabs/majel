...
import { ConfigService, mergeConfig } from '@majel/core'
import { AssetFragment } from '@majel/core/e2e/graphql/generated-e2e-admin-types'
...
describe('AssetServerPlugin', () => {
	...
	describe('unexpected input', () => {
		...
		// https://github.com/majel-ecommerce/majel/security/advisories/GHSA-r9mq-3c9r-fmjq
		describe('path traversal', () => {
			...
			function testPathTraversalOnUrl(urlPath: string) {
				return async () => {
					...
					expect(result).not.toContain('@majel/asset-server-plugin')
	...
	describe('deletion', () => {
		it('deleting Asset deletes binary file', async () => {
			const { deleteAsset } = await adminClient.query<DeleteAssetMutation, DeleteAssetMutationVariables>(
				DELETE_ASSET,
				{
					input: {
						assetId: asset.id,
	...
	// https://github.com/majel-ecommerce/majel/issues/1563
