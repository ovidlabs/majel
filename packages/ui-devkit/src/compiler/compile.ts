...
import { AdminUiAppConfig, AdminUiAppDevModeConfig } from '@majel/common/lib/shared-types'
...
function runWatchMode({
	...
	const devkitPath = require.resolve('@majel/ui-devkit')
		...
		new Promise<void>(async (resolve, reject) => {
			...
			if (watcher) {
				...
				const allGlobalStyles = globalStylesExtensions.reduce(
					(defs, e) => [...defs, ...(Array.isArray(e.globalStyles) ? e.globalStyles : [e.globalStyles])],
					[] as string[],
				)
