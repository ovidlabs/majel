...
import { InternalServerError, Logger } from '@majel/core'
import {
	getNamedType,
	getNullableType,
	GraphQLSchema,
		...
		return {
			didResolveOperation: async ({ request, document }) => {
				...
				if (this.options.logComplexityScore === true) {
					Logger.debug(`Calculating complexity of "${request.operationName ?? 'anonymous'}"`, loggerCtx)
				}
				const complexity = getComplexity({
					...
					estimators: this.options.queryComplexityEstimators ?? [
						defaultMajelComplexityEstimator(
 ...
 */
export function defaultMajelComplexityEstimator(
