...
import { DEFAULT_AUTH_TOKEN_HEADER_KEY, DEFAULT_CHANNEL_TOKEN_KEY } from '@majel/common/lib/shared-constants'
import {
	AdminUiAppConfig,
...
export interface AdminUiPluginOptions {
	 ...
	 * the Majel server is running on.
	 */
	port: number
	/**
	 * @description
	...
	/**
	 * @description
	 * Allows the contents of the `majel-ui-config.json` file to be set, e.g.
	 * for specifying the Majel GraphQL API host, available UI languages, etc.
	 */
...
/**
 * @description
 * This plugin starts a static server for the Admin UI app, and proxies it via the `/admin/` path of the main Majel server.
 *
 ...
 * ## Installation
 *
 * `yarn add \@majel/admin-ui-plugin`
 *
 * or
 *
...
export class AdminUiPlugin implements NestModule {
	...
	async configure(consumer: MiddlewareConsumer) {
		...
		const adminUiConfigPath = path.join(adminUiAppPath, 'majel-ui-config.json')
	...
	private getAdminUiConfig(partialConfig?: Partial<AdminUiConfig>): AdminUiConfig {
		...
		return {
			...
			hideMajelBranding: propOrDefault(
				'hideMajelBranding',
				options.adminUiConfig?.hideMajelBranding || false,
			),
	...
	/**
	 * Overwrites the parts of the admin-ui app's `majel-ui-config.json` file relating to connecting to
	...
	private async overwriteAdminUiConfig(adminUiConfigPath: string, config: AdminUiConfig) {
		...
		try {
			...
			throw new Error(
				'[AdminUiPlugin] Could not write majel-ui-config.json file:\n' + JSON.stringify(e.message),
			)
		}
		Logger.verbose('Applied configuration to majel-ui-config.json file', loggerCtx)
	}
