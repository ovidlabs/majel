...
import { ConfigService, Logger, Order, RequestContext, TransactionalConnection, TtlCache } from '@majel/core'
import {
	Duration,
	endOfDay,
	getDayOfYear,
...
export class MetricsService {
	...
	async loadData(
		...
		while (hasMoreOrders) {
			...
			orders.push(...items)
			Logger.verbose(
				`Fetched orders ${skip}-${skip + take} for channel ${ctx.channel.token} for ${interval} metrics`,
				loggerCtx,
			)
			skip += items.length
