...
const createEnvVar: import('@majel/common/lib/shared-constants').CREATING_MAJEL_APP = 'CREATING_MAJEL_APP'
...
void createMajelApp(projectName, options.useNpm, options.logLevel || 'silent', options.ci)

export async function createMajelApp(
	...
	intro(`Let's create a ${pc.blue(pc.bold('Majel App'))} ✨ ${pc.dim(`v${packageJson.version as string}`)}`)
	...
	if (scaffoldExists) {
		console.log(
			pc.yellow(
				'It appears that a new Majel project scaffold already exists. Re-using the existing files...',
			),
		)
		console.log()
	}
	...
	setupSpinner.start(
		`Setting up your new Majel project in ${pc.green(root)}\nThis may take a few minutes...`,
	)
	...
	const configFile = srcPathScript('majel-config')
	...
	populateSpinner.start(`Initializing your new Majel server`)
	// register ts-node so that the config file can be loaded
	// eslint-disable-next-line @typescript-eslint/no-var-requires
	require(path.join(root, 'node_modules/ts-node')).register()

	try {
		const { populate } = await import(path.join(root, 'node_modules/@majel/core/cli/populate'))
		const { bootstrap, DefaultLogger, LogLevel, JobQueueService } = await import(
			path.join(root, 'node_modules/@majel/core/dist/index')
		)
		...
		const majelLogLevel =
			logLevel === 'silent' ? LogLevel.Error : logLevel === 'verbose' ? LogLevel.Verbose : LogLevel.Info
		...
		const bootstrapFn = async () => {
			...
			const _app = await bootstrap({
				...
				logger: new DefaultLogger({ level: majelLogLevel }),
				importExportOptions: {
	...
	const nextSteps = [
		`${pc.green('Success!')} Created a new Majel server at:`,
		`\n`,
		pc.italic(root),
		`\n`,
 ...
 * a new Majel project in the given location.
 */
function runPreChecks(name: string | undefined, useNpm: boolean): name is string {
	if (typeof name === 'undefined') {
		...
		console.log(`  ${pc.cyan(program.name())} ${pc.green('my-majel-app')}`)
		process.exit(1)
		return false
	}
...
/**
 * Generate the default directory structure for a new Majel project
 */
...
async function copyEmailTemplates(root: string) {
	const templateDir = path.join(root, 'node_modules/@majel/email-plugin/templates')
	try {
