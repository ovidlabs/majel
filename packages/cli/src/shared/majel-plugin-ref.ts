...
export class MajelPluginRef {
	...
	getMetadataOptions() {
		const pluginDecorator = this.classDeclaration.getDecorator('MajelPlugin')
		if (!pluginDecorator) {
			throw new Error('Could not find MajelPlugin decorator')
		}
		const pluginOptions = pluginDecorator.getArguments()[0]
		if (!pluginOptions || !Node.isObjectLiteralExpression(pluginOptions)) {
			throw new Error('Could not find MajelPlugin options')
		}
	...
	getEntities(): EntityRef[] {
		...
		return entitySourceFiles
			.map(sourceFile => sourceFile.getClasses().filter(c => c.getExtends()?.getText() === 'MajelEntity'))
			.flat()
