...
export class PackageJson {
	private _majelPackageJsonPath: string | undefined
	private _rootPackageJsonPath: string | undefined
	constructor(private readonly project: Project) {}

	get majelPackageJsonPath() {
		return this.locatePackageJsonWithMajelDependency()
	}
	...
	determineMajelVersion(): string | undefined {
		const packageJson = this.getPackageJsonContent()
		return packageJson.dependencies['@majel/core']
	}
	...
	getPackageJsonContent() {
		const packageJsonPath = this.locatePackageJsonWithMajelDependency()
		if (!packageJsonPath || !fs.existsSync(packageJsonPath)) {
			note(
				`Could not find a package.json in the current directory. Please run this command from the root of a Majel project.`,
			)
			return false
		}
	...
	addScript(scriptName: string, script: string) {
		...
		const packageJsonPath = this.majelPackageJsonPath
		if (packageJsonPath) {
	...
	locatePackageJsonWithMajelDependency() {
		if (this._majelPackageJsonPath) {
			return this._majelPackageJsonPath
		}
		const rootDir = this.getPackageRootDir().getPath()
		...
		if (this.hasMajelDependency(rootPackageJsonPath)) {
			return rootPackageJsonPath
		}
		for (const dir of potentialMonorepoDirs) {
			...
			for (const subDir of fs.readdirSync(monorepoDir)) {
				...
				if (this.hasMajelDependency(packageJsonPath)) {
					this._majelPackageJsonPath = packageJsonPath
	...
	private hasMajelDependency(packageJsonPath: string) {
