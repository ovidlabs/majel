...
import { MajelPluginRef } from './majel-plugin-ref'

export async function analyzeProject(options: {
	providedMajelPlugin?: MajelPluginRef
	cancelledMessage?: string
}) {
	const providedMajelPlugin = options.providedMajelPlugin
	let project = providedMajelPlugin?.classDeclaration.getProject()
	let tsConfigPath: string | undefined

	if (!providedMajelPlugin) {
...
export async function selectPlugin(project: Project, cancelledMessage: string): Promise<MajelPluginRef> {
	...
	return new MajelPluginRef(targetPlugin as ClassDeclaration)
}

export async function selectEntity(plugin: MajelPluginRef): Promise<EntityRef> {
...
export async function selectMultiplePluginClasses(
	...
	if (selectAll === 'all') {
		return pluginClasses.map(pc => new MajelPluginRef(pc))
	}
	...
	return (targetPlugins as ClassDeclaration[]).map(pc => new MajelPluginRef(pc))
}
...
export function getServices(project: Project): ServiceRef[] {
	const servicesSourceFiles = project.getSourceFiles().filter(sf => {
		return (
			sf.getDirectory().getPath().endsWith('/services') || sf.getDirectory().getPath().endsWith('/service')
		)
	})
