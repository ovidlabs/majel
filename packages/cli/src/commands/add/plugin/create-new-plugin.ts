...
export const createNewPluginCommand = new CliCommand({
	...
	category: 'Plugin',
	description: 'Create a new Majel plugin',
	run: createNewPlugin,
})
...
export async function createNewPlugin(): Promise<CliCommandReturnVal> {
	...
	intro('Adding a new Majel plugin!')
	...
	configSpinner.start('Updating MajelConfig...')
	await pauseForPromptDisplay()
	const majelConfig = new MajelConfigRef(project)
	majelConfig.addToPluginsArray(`${plugin.name}.init({})`)
	addImportsToFile(majelConfig.sourceFile, {
		moduleSpecifier: plugin.getSourceFile(),
		namedImports: [plugin.name],
	})
	await majelConfig.sourceFile.getProject().save()
	configSpinner.stop('Updated MajelConfig')
...
export async function generatePlugin(
...
): Promise<{ plugin: MajelPluginRef; modifiedSourceFiles: SourceFile[] }> {
	...
	constantsFile.getVariableDeclaration('loggerCtx')?.set({ initializer: `'${templateContext.pluginName}'` })

	projectSpinner.stop('Generated plugin scaffold')
	await project.save()
	return {
		modifiedSourceFiles: [pluginFile, typesFile, constantsFile],
		plugin: new MajelPluginRef(pluginClass),
	}
}
