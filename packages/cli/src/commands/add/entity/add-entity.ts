...
async function addEntity(
	...
	const { entityClass, translationClass } = createEntity(majelPlugin, context)
	addEntityToPlugin(majelPlugin, entityClass)
	modifiedSourceFiles.push(entityClass.getSourceFile())
	if (context.features.translatable) {
		addEntityToPlugin(majelPlugin, translationClass)
...
function createEntity(plugin: MajelPluginRef, options: AddEntityOptions) {
	...
	if (!options.features.translatable) {
		...
		translationFile.getImportDeclaration('./entity.template')?.setModuleSpecifier(`./${options.fileName}`)
	}
