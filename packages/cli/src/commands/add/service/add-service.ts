...
async function addService(
	...
	const providedMajelPlugin = providedOptions?.plugin
	const { project } = await analyzeProject({ providedMajelPlugin, cancelledMessage })
	const majelPlugin = providedMajelPlugin ?? (await selectPlugin(project, cancelledMessage))
	...
	if (type === 'entity') {
		...
		try {
			entityRef = await selectEntity(majelPlugin)
		} catch (e: any) {
			if (e.message === Messages.NoEntitiesFound) {
				log.info(`No entities found in plugin ${majelPlugin.name}. Let's create one first.`)
				const result = await addEntityCommand.run({ plugin: majelPlugin })
	...
	if (options.type === 'basic') {
		...
		const serviceSourceFilePath = getServiceFilePath(majelPlugin, options.serviceName)
	...
	const pluginOptions = majelPlugin.getPluginOptions()
	if (pluginOptions) {
	...
	majelPlugin.addProvider(options.serviceName)
	addImportsToFile(majelPlugin.classDeclaration.getSourceFile(), {
...
function getServiceFilePath(plugin: MajelPluginRef, serviceName: string) {
